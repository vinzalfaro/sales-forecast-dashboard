...
# %%
in_df = pd.read_pickle("data/processed/clean_monthly_df.pickle")
...
for item in in_df["ITEM_DUMMY_ID"].unique():
    ...
    plt.savefig(f"results/forecasts/plots/{item}.png")
...
y_pred_df_pivot = (
    ...
    .reset_index()
).to_csv("results/forecasts/test_data_forecasts.csv", index=False)
...
).reset_index().to_csv("results/forecasts/test_data_model_metrics.csv", index=False)

# %% [markdown]
# ## Plot 2021 Holt's Forecasts
...
for item in in_df["ITEM_DUMMY_ID"].unique():
    ...
    plt.savefig(f"results/forecasts/plots_2021/{item}.png")
...
# %%
y_2021_df_pivot.to_csv("results/forecasts/Holt_2021_Forecast.csv", index=False)

# %% [markdown]
# EOQ and ROP for 2021
...
# %%
y_2021_annual.to_csv("results/EOQ_ROP_2021_annual.csv", index=False)